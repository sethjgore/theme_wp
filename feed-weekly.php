...
<channel>
 ...
 <?php while( $weekly->have_posts() ) : $weekly->the_post(); ?>
      ...
      <pubDate><?php $time = get_the_time('D, j M Y h:i:s'); $z = $get_option('timezone_string'); $zone=$z->format('T'); echo $time." ".$zone; ?></pubDate>
      ...
      ?>
        <item>
          <link><?php echo get_post_permalink($event->ID) ?></link>
           <title><![CDATA[<?php echo $event->title; ?> • <?php echo the_field('date', $event->ID) ?>]]></title>
           <source url="<?php bloginfo_rss( 'url' ); ?>"><?php timed($start,$end);?>
        </source>
